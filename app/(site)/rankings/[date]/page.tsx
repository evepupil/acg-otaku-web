...
export default async function RankingDetailPage({
  ...
  const [pick, { picks: summaries }] = await Promise.all([
    getDailyPickByDate(date, 'ranking_pick'),
    getPublishedDailyPickSummaries(1, 366, 'ranking_pick'),
  ])
    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-4">
        <div className="text-center">
          <TrendingUp className="mx-auto h-14 w-14 text-slate-300" />
          <p className="mt-4 text-lg text-slate-400">未找到该日期的排行精选</p>
          <Link href="/rankings" className="mt-3 inline-block text-emerald-600 hover:underline">
            返回列表
          </Link>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 pt-10 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto inline-flex items-center gap-2">
            {previous ? (
              <Link
                href={`/rankings/${previous.pickDate}`}
