...
interface ArtistFeatureData {
  id: number; artistId: string; artistName: string; artistAvatar: string; artistBio: string
  featureTitle: string; featureContent: string; coverPid: string; pixivUrl: string; twitterUrl: string
  isPublished: boolean; artworks: FeatureArtwork[]
}
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={feature.artistName} onChange={(e) => setFeature({ ...feature, artistName: e.target.value })}
          ...
          <div>
            ...
            <input type="text" value={feature.featureTitle} onChange={(e) => setFeature({ ...feature, featureTitle: e.target.value })}
              className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Pixiv主页</label>
            <input type="text" value={feature.pixivUrl} onChange={(e) => setFeature({ ...feature, pixivUrl: e.target.value })} placeholder="https://www.pixiv.net/users/..."
              className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Twitter</label>
            <input type="text" value={feature.twitterUrl} onChange={(e) => setFeature({ ...feature, twitterUrl: e.target.value })} placeholder="https://twitter.com/..."
              className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-sm" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">画师简介</label>
          <textarea value={feature.artistBio} onChange={(e) => setFeature({ ...feature, artistBio: e.target.value })} rows={2}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
        ...
        <textarea
          ...
          rows={12}
          placeholder="撰写画师鉴赏文章，支持Markdown格式..."
