...
import { addArtistFeatureArtwork, removeArtistFeatureArtwork, getArtistFeatureById } from '@/lib/turso'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await addArtistFeatureArtwork(parseInt(id), pid, sortOrder || 0, editorComment)
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await removeArtistFeatureArtwork(parseInt(id), pid)
    return NextResponse.json({ success: true })
  } catch (error) {
